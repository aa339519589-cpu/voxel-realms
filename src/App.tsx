...
function newWorldConfig(name: string, seed: string, mode: GameMode): WorldConfig {
  const now = Date.now();
  return { id: `world-${now.toString(36)}`, name: name.trim() || "新世界", seed: seed.trim() || randomSeed(), mode, createdAt: now, updatedAt: now };
}

function previewConfig(): WorldConfig {
  return { id: "preview", name: "远野", seed: "VOXEL-REALMS-2026", mode: "survival", createdAt: 0, updatedAt: 0 };
}
...
function InventoryScreen({ mode, items, selected, query, onQuery, onSelect, onCraft, onClose }: { mode: GameMode; items: Array<{ block: BlockId; count: number; name: string }>; selected?: BlockId; query: string; onQuery: (value: string) => void; onSelect: (block: BlockId) => void; onCraft: (recipeId: string) => boolean | undefined; onClose: () => void }) {
  ...
  return <section className="inventory-screen overlay dark-overlay"><div className="inventory-panel"><header><div><span className="kicker">{mode === "creative" ? "CREATIVE CATALOG" : "PLAYER INVENTORY"}</span><h2>物品与方块</h2></div><button className="icon-control" title="关闭背包" onClick={onClose}><X size={19} /></button></header><label className="inventory-search"><Search size={17} /><input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="搜索方块" /></label><div className="inventory-grid">{items.map((item) => <button key={item.block} className={selected === item.block ? "inventory-slot selected" : "inventory-slot"} onClick={() => onSelect(item.block)} title={item.name}><BlockSwatch block={item.block} /><span>{item.name}</span><b>{item.count < 0 ? "∞" : item.count}</b></button>)}</div>{mode === "survival" ? <div className="crafting-strip"><span className="kicker">2×2 CRAFTING</span><div>{CRAFTING_RECIPES.map((recipe) => { const ready = recipe.ingredients.every((item) => (counts.get(item.block) ?? 0) >= item.count); return <button key={recipe.id} disabled={!ready} onClick={() => onCraft(recipe.id)} title={recipe.name}><span className="recipe-inputs">{recipe.ingredients.map((item) => <i key={item.block}><BlockSwatch block={item.block} /><b>{item.count}</b></i>)}</span><span className="recipe-arrow">→</span><i><BlockSwatch block={recipe.output.block} /><b>{recipe.output.count}</b></i></button>; })}</div></div> : null}<footer><span>{mode === "creative" ? `${ALL_BLOCKS.length - 1} 种方块` : `${items.length} 个物品堆叠`}</span><button className="menu-primary" onClick={onClose}><Check size={17} />完成</button></footer></div></section>;
}
